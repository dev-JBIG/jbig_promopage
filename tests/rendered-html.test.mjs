...
test("keeps recruitment visuals lightweight, responsive, and reduced-motion aware", async () => {
  ...
  assert.match(recruitCss, /\.testimonial-story\s*\{[^}]*--testimonial-card-padding:\s*clamp\(34px,\s*4vw,\s*52px\)[^}]*grid-template-columns:\s*repeat\(12,\s*minmax\(0,\s*1fr\)\)/s);
  assert.doesNotMatch(recruitCss, /\.testimonial-story\s*\{[^}]*border-top\s*:/s);
  assert.match(recruitCss, /\.testimonial-author\s*\{[^}]*grid-column:\s*2\s*\/\s*span\s+2[^}]*padding-top:\s*var\(--testimonial-card-padding\)/s);
  assert.match(recruitCss, /\.testimonial-author-information strong\s*\{[^}]*font-size:\s*clamp\(23px,\s*1\.95vw,\s*26px\)/s);
  assert.match(recruitCss, /\.testimonial-author-information span\s*\{[^}]*font-size:\s*clamp\(18px,\s*1\.5vw,\s*20px\)/s);
  assert.match(recruitCss, /\.testimonial-message-shell\s*\{[^}]*grid-column:\s*4\s*\/\s*-1/s);
  ...
  assert.match(recruitCss, /@media\s*\(max-width:\s*760px\)[\s\S]*\.testimonial-author\s*\{[^}]*grid-column:\s*1[^}]*padding-top:\s*0/s);
